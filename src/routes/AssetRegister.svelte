<script>
    ...
    async function setAssetClasses() {
        receipts = await Promise.all(
            tempReceipts.map(async (r) => {
                let information = r.receipt.receiptInformations[0]?.information
                    ? cborDecode(
                          r.receipt.receiptInformations[0]?.information.slice(
                              18
                          )
                      )
                    : null;
        ...
        filteredReceipts = receipts.filter(
            (r, index) => index > skip && index < perPage * currentPage
        );
    }
    ...
    function goToAssetInformation(receipt) {
        ...
        localStorage.setItem(
            "selectedReceiptSchema",
            $selectedReceipt.schema.hash
        );
        navigate(
            `#asset-information/${$selectedReceipt.receipt.receiptId}/${receipt.receipt.receiptInformations[0].id}`
        );
    }
    ...
    async function getAuditHistory() {
        ...
        if ($vault.address) {
            loading = true;
            try {
                let data = await getSubgraphData(
                    $activeNetwork,
                ...
                <tbody>
                    {#if receipts.length}
                        {#each filteredReceipts as receipt}
                            <tr class="tb-row">
                                <td
                                    class="brown hover-underline cursor-pointer receipt-{receipt
                                        .receipt.receiptId}"
                                    on:click={() => {
                                        goToAssetInformation(receipt);
                                    }}>{receipt.receipt.receiptId}</td
                                >
                                <td class="asset-class-cell"
                                    >{receipt.schema?.displayName || ""}</td
                                >
                                <td
